...
class DeadlineSystem(object):
    ...
    # Convert to csv
    def calCsv(self, id, deadlines):
        with open("tempfiles/" + id + "_calendar.csv", 'w') as csvFile:
    ...
    def calIcal(self, id):
        convert = Convert()
        csv_file_location = "tempfiles/" + id + "_calendar.csv"
        ical_file_location = "calendars/" + id + "_calendar.ics"
        csv_configs = {
        ...
        os.remove("tempfiles/" + id + "_calendar.csv")

    def createCalender(self, id, deadlines, outFile):

        self.calCsv(id, deadlines)
        if outFile == 'ical':
            self.calIcal(id)
